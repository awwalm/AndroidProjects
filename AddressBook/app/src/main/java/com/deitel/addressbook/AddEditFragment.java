/*
* AddEditFragment (Section 9.12)—This class manages the TextInputLayouts and a
*  FloatingActionButton for adding a new contact or editing and existing one.
* AddEditFragment’s nested interface defines a callback method implemented by MainActivity
...
public class AddEditFragment extends Fragment implements LoaderManager.LoaderCallbacks<Cursor>
{
    ...
    public View onCreateView(LayoutInflater inflater,ViewGroup container,Bundle savedInstanceState)
    {
       ...
       coordinatorLayout = (CoordinatorLayout) getActivity().findViewById(R.id.coordinatorLayout);
        ...
        if (contactUri != null)
        {
            getLoaderManager().initLoader(CONTACT_LOADER, null,
    ...
    private void saveContact()
    {
        ...
        contentValues.put
                (Contact.COLUMN_NAME, nameTextInputLayout.getEditText().getText().toString());
        contentValues.put
                (Contact.COLUMN_PHONE, phoneTextInputLayout.getEditText().getText().toString());
        contentValues.put
                (Contact.COLUMN_EMAIL, emailTextInputLayout.getEditText().getText().toString());
        contentValues.put
                (Contact.COLUMN_STREET, streetTextInputLayout.getEditText().getText().toString());
        contentValues.put
                (Contact.COLUMN_CITY, cityTextInputLayout.getEditText().getText().toString());
        contentValues.put
                (Contact.COLUMN_STATE, stateTextInputLayout.getEditText().getText().toString());
        contentValues.put
                (Contact.COLUMN_ZIP, zipTextInputLayout.getEditText().getText().toString());
        ...
        if (addingNewContact)
        {
            ...
            {
                Snackbar.make
                    (coordinatorLayout, R.string.contact_added, Snackbar.LENGTH_LONG).show();
                listener.onAddEditCompleted(newContactUri);
            }
            else
            {
                Snackbar.make
                    (coordinatorLayout, R.string.contact_not_added, Snackbar.LENGTH_LONG).show();
            }
        }
        else
        {   // use Activity's ContentResolver to invoke
            ...
            if (updatedRows > 0)
            {
                ...
                Snackbar.make
                    (coordinatorLayout, R.string.contact_updated, Snackbar.LENGTH_LONG).show();
            }
            else
            {
                Snackbar.make
        ...
        switch (id)
        {
            case CONTACT_LOADER:
                return new androidx.loader.content.CursorLoader
                        (getActivity(), contactUri, // Uri of contact to display
